---
name: nice_insights_metrics
description: Use when answering analytics questions against the Nice Insights ecommerce metrics MCP server, especially ad, order, order line, cohort, timeseries, or email metrics. Covers ecommerce metrics like ad spend, impressions, clicks, CAC, sales, refunds, order counts, margins, retention, LTV, blended CAC, email profile counts, email-attributed sales, email event volume, and other ecommerce reporting. Before any order, order-line, or cohort query, ask whether to include or exclude refunds.
---
